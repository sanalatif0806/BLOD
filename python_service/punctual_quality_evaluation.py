...
class PunctualQualityEvaluation:
    ...
    def accessibility_stats(self):
        ...
        all_up = self.analysis_data[(self.analysis_data['Sparql endpoint'] == 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] == 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] == 'VoID file available') & (
                                                self.analysis_data[
                                                    'Availability of a common accepted Media Type'] == 'True')].shape[0]
        all_down = self.analysis_data[(self.analysis_data['Sparql endpoint'] != 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] != 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] != 'VoID file available')].shape[
            0]
        only_sparql = self.analysis_data[(self.analysis_data['Sparql endpoint'] == 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] != 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] != 'VoID file available')].shape[
            0]
        only_dump = self.analysis_data[(self.analysis_data['Sparql endpoint'] != 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] == 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] != 'VoID file available') & (
                                                   self.analysis_data[
                                                       'Availability of a common accepted Media Type'] == 'True')].shape[
            0]
        only_void = self.analysis_data[(self.analysis_data['Sparql endpoint'] != 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] != 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] == 'VoID file available')].shape[
            0]
        sparql_dump = self.analysis_data[(self.analysis_data['Sparql endpoint'] == 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] == 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] != 'VoID file available') & (
                                                     self.analysis_data[
                                                         'Availability of a common accepted Media Type'] == 'True')].shape[
            0]
        sparql_void = self.analysis_data[(self.analysis_data['Sparql endpoint'] == 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] != 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] == 'VoID file available')].shape[
            0]
        dump_void = self.analysis_data[(self.analysis_data['Sparql endpoint'] != 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] == 1) & (self.analysis_data[
                                                                                           'Availability VoID file'] == 'VoID file available') & (
                                                   self.analysis_data[
                                                       'Availability of a common accepted Media Type'] == 'True')].shape[
            0]
        sparql_dump_down = self.analysis_data[(self.analysis_data['Sparql endpoint'] != 'Available') & (
                    self.analysis_data['Availability of RDF dump (metadata)'] != 1)].shape[0]
        sparql_or_dump_UP = self.analysis_data[(self.analysis_data['Sparql endpoint'] == 'Available') | (
                    self.analysis_data['Availability of RDF dump (metadata)'] == 1) & (self.analysis_data[
                                                                                           'Availability of a common accepted Media Type'] == True)].shape[
            0]
        ...
        with open(f'../data/evaluation_results/2024/{self.output_dir}/punctual/availability_stats.csv', mode='w',
                  newline='') as file:
    ...
    def write_data_on_csv(self, metric, pandas_df, index=True):
        ...
        save_path = os.path.join(here,
                                 f'../data/evaluation_results/2024/{self.output_dir}/punctual/{metric}_evaluation.csv')
    ...
    def get_kgs_available_with_license(self):
        ...
        df = self.analysis_data[
            ...
            (
                    ((self.analysis_data['License machine redeable (metadata)'] != '-') & (
                                self.analysis_data['License machine redeable (metadata)'] != 'False')) |
                    (self.analysis_data['License machine redeable (query)'] != '-')
            )
            ]

        self.write_data_on_csv('availability_and_license',
                               df[['Sparql endpoint', 'Availability of RDF dump (metadata)', 'Availability VoID file',
                                   'Availability of a common accepted Media Type',
                                   'License machine redeable (metadata)', 'License machine redeable (query)']],
                               index=False)
    ...
    def generate_stats(self, metrics, output_filename, only_sparql_up=True):
        ...
        here = os.path.dirname(os.path.abspath(__file__))
        save_path = os.path.join(here,
                                 f'../data/evaluation_results/2024/{self.output_dir}/punctual/{output_filename}.csv')
